    ...
    if (TIM>=12) 
       { 
          ...
          read_sensor_values_F(); 
           for(SP_BEGIN = 0  ; SP_BEGIN  < SPL; SP_BEGIN  += 2) 
               {
                  read_sensor_values_F();            
                  P = error;
                  I = I + previous_I;
         ...
         {  
       ...
       else if (CH_SENSOR=='p')
           {
                while(1)
               {
                   ...
                   if ((A6<200)||(A7<200))
                      {
                         Motor(SPL,SPR);  while(analogRead(PA2)>1000){} Motor(20,20,10); 
                         Motor(SPL,SPR);  while(analogRead(PA2)<1000){}      
                        
                                
                               goto JP5;                          
                       }
                    else if ((A0<200)||(A0<200))
                       {
                            Motor(SPL,SPR);  while(analogRead(PA3)>1000){} Motor(20,20,10);
                            Motor(SPL,SPR);  while(analogRead(PA3)<1000){} 
              ...
              while(1)
                  {
                     read_sensor_F();  
                      if ((A0>=200)&&(A7<200))
                        {
                           do  {Motor(15,15);}  while(digitalRead(PA11)==1); 
                           Motor(-50,-50,RT);
                           Motor(0,0,1); 
                           goto JP5;  
                        }                      
                        ...
                        {
                           do  {Motor(15,15);}  while(digitalRead(PA10)==1);  
                           Motor(-50,-50,RT);
                           Motor(0,0,1); 
                           goto JP6;
                        }         
